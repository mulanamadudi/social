...
You need to create and seed the database for Prisma. Run the `setup` script in `package.json` using your preferred package manager.
The `start` script runs this automatically through an npm `prestart` hook, but you can also run `npm run setup` manually during deployment. The `setup` script now applies migrations and seeds the initial platform data so the app can create social accounts without foreign key errors.
